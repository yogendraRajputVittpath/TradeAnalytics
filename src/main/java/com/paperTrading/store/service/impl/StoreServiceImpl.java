...
public class StoreServiceImpl implements StoreService {
    ...
    public ApiResponse<?> getAllProducts() {
        ...
        response.put("items",productRepository.findByStockTrue());
        return ApiResponse.builder().status("SUCCESS").message("Products fetched successfully").data(response).build();
    }
    ...
    public ApiResponse<?> placeOrder(PlaceOrderRequest request) {
        ...
        for (OrderItemRequest item :request.getItems()) {
            ...
            StoreOrderItem orderItem =StoreOrderItem.builder().productId(product.getId()).productName(product.getName()).productPrice(product.getPrice())
                            .storeOrder(storeOrder).build();
            orderItems.add(orderItem);
        }
        if (user.getCoins() < totalAmount) {
            throw new InsufficientCoinsException("Insufficient coins");
        }
        user.setCoins(user.getCoins() - totalAmount);
        ...
        return ApiResponse.builder().status("SUCCESS").message("Order placed successfully").data(response).build();
    }

    @Override
    public Object getOrders(String token) {
        ...
        List<StoreOrder> orders =storeOrderRepository.findByUserIdOrderByIdDesc(user.getId());
